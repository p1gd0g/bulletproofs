...
impl RangeProof {
    ...
    pub fn verify<R: Rng + CryptoRng>(
        ...
        for V in value_commitments.iter() {
            transcript.commit_point(&V.compress());
        }
        transcript.commit_point(&self.A);
        transcript.commit_point(&self.S);
        ...
        transcript.commit_point(&self.T_1);
        transcript.commit_point(&self.T_2);

        let x = transcript.challenge_scalar();

        transcript.commit_scalar(&self.t_x);
        transcript.commit_scalar(&self.t_x_blinding);
        transcript.commit_scalar(&self.e_blinding);
